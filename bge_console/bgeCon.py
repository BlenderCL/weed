...
def draw_console_gl():
    ...
    bgl.glLoadIdentity()
    bgl.gluOrtho2D(0, render.getWindowWidth(),
                   0, render.getWindowHeight())
    bgl.glMatrixMode(bgl.GL_MODELVIEW)
    bgl.glLoadIdentity()
    # text BLF drawing routine
    for r in logic.rows:
        bgl.glColor4f(r['color'][0], r['color'][1], 
                      r['color'][2] ,1 - r['alpha'])
    ...
    def handleEvents(self, evn_handled, key_repeat, repeat_timer, repeat_delay):
        ...
        for key, status in evn_handled:
            if (key in modifiers and
                status in (logic.KX_INPUT_ACTIVE,
...
def main(cont):
    ...
    if sens.positive :
        ...
        if hasattr(logic, '__console__'):
            ...
            (bcon.key_repeat,
             own['repeat_timer']) = bcon.handleEvents(sens.events,
                                                bcon.key_repeat,
                                                own['repeat_timer'],
                                                own['repeat_delay'])
